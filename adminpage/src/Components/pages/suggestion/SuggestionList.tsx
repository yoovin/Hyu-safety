...
const SuggestionList = (props: Props) => {
    ...
    const getNoticeData = (query?: any) => {
        axios.get('/suggestion', {params: {...query, reverse: noticeReverse, deleted: false}})
        .then(res => {
  ...
  return (
	...
	<div className="mb-2">
        <span className="text-2xl font-semibold leading-tight">건의사항</span>
			...
			<tbody className="border-b bg-gray-100 border-gray-100">
                {notices && notices.map(item => (
					...
					<td className="px-3 py-2 text-left">
                        <Link to={`/suggestion/${item.index}`}>{item.preview}</Link>
					</td>
